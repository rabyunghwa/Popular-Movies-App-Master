...
public class TopRatedMoviesUpdaterService extends IntentService {

    private static final String TAG = "TopRatedMoviesUpdaterService";

    public TopRatedMoviesUpdaterService() {
        super(TAG);
    }
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        Intent intentTopRated = new Intent(GlobalConsts.TOP_RATED_MOVIES_FETCHED);

        try {
            // Delete old items so that we dont overload the layout with same data
            Uri dirUriItemsContract = ItemsContract.HighestRatedEntry.buildDirUri();
            ...
            if (hasNewItems) {
                ...
                array = RemoteEndpointUtil.fetchTopRatedMovieListJsonArray(page);

                if (array == null) {
                    intentTopRated.putExtra("fetchNewItemsResult", false);
                ...
                if (array != null) {
                    ...
                    for (int i = 0; i < array.length(); i++) {
                        ...
                        cursorCheck = getContentResolver().query(ItemsContract.HighestRatedEntry.buildItemUri(movieId), null, null, null, null);

                        if (!cursorCheck.moveToFirst()) {
                            values.put(TopRatedMoviesLoader.Query.COL_NAME_MOVIE_ID, object.getString("id"));
                            LogUtil.log_i(TAG, "inserting top rated movie data movie id: " + object.getString("id"));
                            values.put(TopRatedMoviesLoader.Query.COL_NAME_ORIGINAL_TITLE, object.getString(TopRatedMoviesLoader.Query.COL_NAME_ORIGINAL_TITLE));
                            LogUtil.log_i(TAG, "inserting top rated movie data original title: " + object.getString(TopRatedMoviesLoader.Query.COL_NAME_ORIGINAL_TITLE));
                            values.put(TopRatedMoviesLoader.Query.COL_NAME_OVERVIEW, object.getString(TopRatedMoviesLoader.Query.COL_NAME_OVERVIEW));
                            LogUtil.log_i(TAG, "inserting top rated movie data overview: " + object.getString(TopRatedMoviesLoader.Query.COL_NAME_OVERVIEW));
                            values.put(TopRatedMoviesLoader.Query.COL_NAME_VOTE_AVERAGE, object.getString(TopRatedMoviesLoader.Query.COL_NAME_VOTE_AVERAGE));
                            LogUtil.log_i(TAG, "inserting top rated movie data vote average: " + object.getString(TopRatedMoviesLoader.Query.COL_NAME_VOTE_AVERAGE));
                            values.put(TopRatedMoviesLoader.Query.COL_NAME_RELEASE_DATE, object.getString(TopRatedMoviesLoader.Query.COL_NAME_RELEASE_DATE));
                            LogUtil.log_i(TAG, "inserting top rated movie data release date: " + object.getString(TopRatedMoviesLoader.Query.COL_NAME_RELEASE_DATE));
                            values.put(TopRatedMoviesLoader.Query.COL_NAME_POSTER_THUMBNAIL, object.getString(TopRatedMoviesLoader.Query.COL_NAME_POSTER_THUMBNAIL));
                            LogUtil.log_i(TAG, "inserting top rated movie data poster thumbnail: " + object.getString(TopRatedMoviesLoader.Query.COL_NAME_POSTER_THUMBNAIL));
                            cVVector.add(values);
                        }
                    ...
                    if (cVVector.size() > 0) {
                        ...
                        int result = getContentResolver().bulkInsert(ItemsContract.HighestRatedEntry.buildDirUri(), cvArray);

                        LogUtil.log_i(TAG, "Top Rated Movies Bulk Insert Result: " + result);
                    }
                }
            }
